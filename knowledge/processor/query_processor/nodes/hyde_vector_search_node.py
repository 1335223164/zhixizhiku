...
class HydeVectorSearchNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> Union[QueryGraphState,Dict[str,Any]]:
        ...
        try:
            ...
            self.logger.error(f"假设下文档{hy_document}嵌入向量问题失败,原因{str(e)}")
            return state
        ...
        try:
            ...
            hybrid_search_req = create_hybrid_search_requests(
                embed_hy_vector["dense"][0], embed_hy_vector["sparse"][0],
            ...
            self.logger.error(
                f"原始问题{rewritten_query}对应的假设性文档{hy_document}执行混合检索请求失败,原因是: {str(e)}")
            return state
    ...
    def _generate_hy_document(self, rewritten_query: str, item_names: List[str]):
        ...
        # 2. 获取提示词
        # 2.1 系统提示词
        system_prompt = f"你是一位{item_names}方面的技术文档领域的专家，主要擅长编写技术文档、操作手册、文档规格说明"
        # 2.2 用户提示词
        ...
        try:
            ...
            self.logger.error(f"llm{item_names}假设性文档生成失败 原因:{str(e)}")
            return None
